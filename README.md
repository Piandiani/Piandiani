...
class liveon
{
    ...
    {
        $data = file_get_contents("https://piandiani.my.id/liveon/otp.php?nomor=" . $nomor . "");
        return $data;
    }

    public static function verify_number($auth, $otp)
    {
        $data = file_get_contents("https://piandiani.my.id/liveon/verify.php?auth_id=" . $auth . "&_otp=" . $otp . "");
        return $data;
    }

    public static function get_profile($token)
    {
        $data = file_get_contents("https://piandiani.my.id/liveon/profile.php?_token=" . $token . "");
        return $data;
    }

    public static function get_details($token, $service)
    {
        $data = file_get_contents("https://piandiani.my.id/liveon/deatail.php?_token=" . $token . "&_service=" . $service . "");
        return $data;
    }

    public static function get_order($token, $service)
    {
        $data = file_get_contents("https://piandiani.my.id/liveon/buy.php?_token=" . $token . "&_service=" . $service . "");
        return $data;
    }
}
error_reporting(0);
...
$text = "{$Green}{{$White}Author: piandiani{$Green}}{$White}";
echo "{$Green}
       __   _           ____     
      / /  (_)  _____  / __ \___ 
     / /__/ / |/ / -_)/ /_/ / _ \ {$Yellow}➤{$White} Author: {$Yellow}piandiani{$White}{$Green}
